      ...
      {(menuItems || []).map((item, idx) => {
        return (
          <Fragment key={item.key + idx}>
            {item.isTitle ? (
              <li className={clsx('menu-label', 'mt-2')}>
              ...
              <>
                {item.children ? (
                  <MenuItemWithChildren
                    item={item}
                    toggleMenu={toggleMenu}
                    className={clsx('nav-item', getActiveClass(item))}
                    linkClassName={clsx('nav-link', getActiveClass(item))}
                    subMenuClassName="nav flex-column"
                    activeMenuItems={activeMenuItems}
                  />
                ) : (
                  <MenuItem item={item} linkClassName={clsx('nav-link', getActiveClass(item))} className={clsx('nav-item', getActiveClass(item))} />
                )}
              </>
            )}
          </Fragment>
