...
const BasicToast = () => {
  ...
  return (
    <ComponentContainerCard title="Basic">
...
const TranslucentToast = () => {
  const { isTrue: isOpen, toggle } = useToggle(true)
  return (
    <ComponentContainerCard title="Translucent">
...
const StackingToast = () => {
  const { isTrue: isOpen, toggle } = useToggle(true)
  const { isTrue: toastOpen, toggle: toastToggle } = useToggle(true)
  return (
    <ComponentContainerCard title="Stacking">
      <div className="bg-light p-3">
...
const CustomContentToast = () => {
  const { isTrue: isOpen, toggle } = useToggle(true)
  const { isTrue: toastOpen, toggle: toastToggle } = useToggle(true)
  const { isTrue: toastOpen2, toggle: toastToggle2 } = useToggle(true)
  return (
    <ComponentContainerCard title="Custom Content">
      <div className="bg-light p-3">
...
const ToastPlacements = () => {
  ...
  const placements: ToastPosition[] = [
    'top-start',
    'top-center',
    'top-end',
    'middle-start',
  ...
  return (
    <ComponentContainerCard title="Toast Placement">
      <form>
        <div className="form-group mb-3">
          <label htmlFor="selectToastPlacement">Toast placement</label>
          <select className="form-select mt-2" onChange={(e) => setPosition(e.currentTarget.value as ToastPosition)}>
      ...
      <div
        ...
        style={{ height: 260, backgroundColor: 'rgba(235, 240, 247, 0.1)' }}>
...
const CustomToast = () => {
  const { isTrue: isOpen, toggle } = useToggle(true)
  return (
    <ComponentContainerCard title="Custom Toast">
      <Toast show={isOpen} onClose={toggle} role="alert" aria-live="assertive" aria-atomic="true">
        ...
        <ToastBody className="text-center">
          <Image alt="user" src={user5} className="d-block mx-auto rounded-circle thumb-xl" />
