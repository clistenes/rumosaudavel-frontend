...
const ContainerCard = ({ name, handle, avatar, children }: { name: string; handle: string; avatar: StaticImageData; children: ReactNode }) => {
  return (
        ...
        <CardBody className="border-dashed-bottom border-theme-color">
          ...
          <div className="d-flex align-items-center">
            <div className="flex-shrink-0">
              <Image src={avatar} alt="avatar" className="rounded-circle thumb-xxl mx-auto d-inline-block" />
            </div>
...
const MovableCard = ({ project }: { project: ProjectType }) => {
  const { clientName, logo, status, title, budget, deadlineDate, members, progress, startDate, tasks } = project
  return (
    <Card>
      <CardBody>
        <div className="position-absolute  end-0 me-3">
          <span
            className={clsx(
              'badge rounded  bg-transparent border',
              status === 'In Progress'
        ...
        <div className={`d-flex align-items-center ${members ? 'mb-3 pb-2 border-dashed-bottom' : ''}`}>
          <div className="flex-shrink-0">
            <Image src={logo} alt="logo" height={50} className="rounded-circle d-inline-block" />
          </div>
          <div className="flex-grow-1 ms-2 text-truncate">
        ...
        {members && (
          <>
            <div className="d-flex justify-content-between fw-semibold align-items-center  my-3">
              <div className="img-group d-flex justify-content-center">
                {members.slice(0, 3).map((member, idx) => (
                  <Link key={idx} className={clsx('user-avatar position-relative d-inline-block', { 'ms-n2': idx != 0 })} href="#">
                    <Image src={member} alt="avatar" className="thumb-md shadow-sm rounded-circle" />
                  </Link>
                ))}
                <Link href="" className="user-avatar position-relative d-inline-block ms-1">
            ...
            <div>
              ...
              <ProgressBar now={progress} variant="secondary" style={{ height: 5 }} />
              <Row className="mt-3 align-items-center">
                <Col xs="auto" md={5}>
...
const ProjectList = () => {
  const [project1, setProject1] = useState<ProjectType[]>(projectData.slice(0, 4))

  const [project2, setProject2] = useState<ProjectType[]>(projectData.slice(4, 8))

  const [project3, setProject3] = useState<ProjectType[]>(projectData.slice(8, 11))
  return (
