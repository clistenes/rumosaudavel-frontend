...
import { getAllProducts, getAllProject } from '@/helpers/data'
import { ProjectsType } from '@/types/data'

const AppBoxCard = ({ all_hours, client, days_left, deadline, logo, progress, start_date, tasks_completed, team, title, today_hours }: ProjectsType) => {
  return (
    <Card>
      <CardBody>
        <div className="position-absolute top-0 start-0 translate-middle"><i className="fas fa-circle text-success" /></div>
        <Row>
          <Col md={6}>
            <div className="d-flex align-items-center">
              ...
              <div className="flex-grow-1 ms-2 text-truncate">
                <h4 className="m-0 fw-semibold text-dark fs-16">{title}</h4>
                <p className="text-muted mb-0 fs-13"><span className="text-dark">Client : </span>{client}</p>
              </div>
            </div>
          </Col>
          <Col md={6} className="text-lg-end">
            <h6 className="fw-semibold m-0">Start : <span className="text-muted fw-normal"> {start_date}</span></h6>
            <h6 className="fw-semibold  mb-0 mt-2">Deadline : <span className="text-muted fw-normal"> {deadline}</span></h6>
          </Col>
        </Row>
        <div className="mt-3">
          <div className="d-flex justify-content-between">
            <h6 className="fw-semibold m-0 align-self-center">All Hours : <span className="text-muted fw-normal"> {all_hours}</span></h6>
            <h6 className="fw-semibold">Today : <span className="text-muted fw-normal"> {today_hours}</span><span className="badge bg-soft-pink text-pink fw-semibold ms-2"><i className="far fa-fw fa-clock" /> {days_left} days left</span></h6>
          </div>
          ...
          <ProgressBar variant='primary' className="mb-3 " now={progress} style={{ height: 3, backgroundColor: 'danger'}} />
          <div className="d-flex justify-content-between">
            <div className="img-group d-flex justify-content-center">
              {
                team.map((img, idx) => (
                  <a key={idx} className="user-avatar position-relative d-inline-block" href="">
