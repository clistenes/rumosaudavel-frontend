// import { ChartData, ChartOptions, type ChartConfiguration } from 'chart.js/auto'

import { ChartData, ChartOptions } from "chart.js";

// // const lineChartConfig: ChartConfiguration = {
// //   type: 'line',
// //   data: {
// //     labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
// //     datasets: [
// //       {
// //         label: 'Monthly Report',
// //         data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
// //         backgroundColor: ['#6f6af8'],
// //         borderColor: ['#6f6af8'],
// //         borderWidth: 2,
// //         borderDash: [3],
// //         borderJoinStyle: 'round',
// //         borderCapStyle: 'round',
// //         pointBorderColor: '#6f6af8',
// //         pointRadius: 3,
// //         pointBorderWidth: 1,
// //         tension: 0.3,
// //       },
// //       {
// //         label: 'Monthly Report',
// //         data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
// //         backgroundColor: ['#16cdc7'],
// //         borderColor: ['#16cdc7'],
// //         borderWidth: 2,
// //         borderDash: [0],
// //         borderJoinStyle: 'round',
// //         borderCapStyle: 'round',
// //         pointBorderColor: '#16cdc7',
// //         pointRadius: 3,
// //         pointBorderWidth: 1,
// //         tension: 0.3,
// //       },
// //     ],
// //   },
// //   options: {
// //     maintainAspectRatio: false,
// //     plugins: {
// //       legend: {
// //         labels: {
// //           color: '#7c8ea7',
// //           font: {
// //             family: 'Be Vietnam Pro',
// //           },
// //         },
// //       },
// //     },
// //     scales: {
// //       y: {
// //         beginAtZero: true,
// //         ticks: {
// //           // Include a dollar sign in the ticks
// //           callback: function (value, index, values) {
// //             return '$' + value
// //           },
// //           color: '#7c8ea7',
// //         },
// //         grid: {
// //           // drawBorder: 'border',
// //           color: 'rgba(132, 145, 183, 0.15)',
// //           // borderDash: [3],
// //           // borderColor: 'rgba(132, 145, 183, 0.15)',
// //         },
// //         // beginAtZero: true,
// //       },
// //       x: {
// //         ticks: {
// //           color: '#7c8ea7',
// //         },
// //         grid: {
// //           display: false,
// //           color: 'rgba(132, 145, 183, 0.09)',
// //           // borderDash: [3],
// //           // borderColor: 'rgba(132, 145, 183, 0.09)',
// //         },
// //       },
// //     },
// //   },
// // }

// export	const chartData: ChartData<'line'> = {
//   labels: ['Jan','Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
//   datasets: [{
//       label: 'Monthly Report',
//       data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
//       backgroundColor: [
//          '#6f6af8',
//       ],
//       borderColor: [
//           '#6f6af8',
//       ],
//       borderWidth: 2,
//       borderDash	:[3],
//       borderJoinStyle: "round",
//       borderCapStyle: "round",
//       pointBorderColor: '#6f6af8',
//       pointRadius: 3,
//       pointBorderWidth: 1,
//       tension: 0.3,
//   },
//   {
//      label: 'Monthly Report',
//      data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
//      backgroundColor: [
//         '#16cdc7',
//      ],
//      borderColor: [
//          '#16cdc7',
//      ],
//      borderWidth: 2,
//      borderDash	:[0],
//      borderJoinStyle: "round",
//      borderCapStyle: "round",
//      pointBorderColor: '#16cdc7',
//      pointRadius: 3,
//      pointBorderWidth: 1,
//      tension: 0.3,
//  }]
// 	};

// 	// chart options
// export	const chartOpts: ChartOptions<'line'> = {
//   maintainAspectRatio: false,
//   plugins: {
//       legend: {
//           labels: {                   
//               color: '#7c8ea7',
//               font:{
//                   family: 'Be Vietnam Pro',
//               }
//           },

//       }
//   }  ,
//   scales: {            
//       y: {
//           beginAtZero: true,
//           ticks: {
//               // Include a dollar sign in the ticks
//               callback: function(value, index, values) {
//                   return '$' + value;
//               },
//               color: '#7c8ea7',
//           },               
//           grid: { 
//               color: 'rgba(132, 145, 183, 0.15)',
//           } ,
//       },
//       x: {   
//         ticks: {
//           color: '#7c8ea7',
//         },            
//           grid: {
//               display: false,
//               color: 'rgba(132, 145, 183, 0.09)',
//           }    
//       }            
//    },

// 	};

// const donutChartConfig: ChartConfiguration = {
//   type: 'doughnut',
//   data: {
//     labels: ['Desktops', 'Laptop', 'Tablets', 'Mobiles'],
//     datasets: [
//       {
//         data: [80, 50, 100, 121],
//         backgroundColor: ["#f67f7f",
//                 "#7777f0",
//                 "#16cdc7",
//                 "#6f6af8",],
//         // cutout: 100,
//         radius: 100,
//         borderColor: 'transparent',
//         borderRadius: 0,
//         hoverBackgroundColor: [ "#4d79f6",
//           "#ff5da0",
//           "#e0e7fd",
//           "#4ac7ec",],
//       },
//     ],
//   },
//   options: {
//     maintainAspectRatio: false,
//     plugins: {
//       legend: {
//         labels: {
//           color: '#7c8ea7',
//           font: {
//             family: 'Be Vietnam Pro',
//           },
//         },
//       },
//     },
//   },
// }

// const barChartConfig: ChartConfiguration = {
//   type: 'bar',
//   data: {
//     labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
//     datasets: [
//       {
//         label: 'Monthly Report',
//         data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
//         borderRadius: 100,
//         borderSkipped: false,
//         backgroundColor: '#6f6af8',
//         borderColor: '#6f6af8',
//         borderWidth: 1,
//         indexAxis: 'x',
//         barThickness: 15,
//         grouped: true,
//         maxBarThickness: 9,
//         barPercentage: 50,
//       },
//       {
//         label: 'Monthly Report',
//         data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
//         borderRadius: 100,
//         borderSkipped: false,
//         backgroundColor: '#16cdc7',
//         borderColor: '#16cdc7',
//         borderWidth: 1,
//         indexAxis: 'x',
//         barThickness: 15,
//         grouped: true,
//         maxBarThickness: 9,
//       },
//     ],
//   },

//   options: {
//     maintainAspectRatio: false,
//     responsive: true,
//     plugins: {
//       legend: {
//         position: 'top',
//         labels: {
//           color: '#7c8ea7',
//           font: {
//             family: 'Be Vietnam Pro',
//           },
//         },
//       },
//       title: {
//         display: false,
//         text: 'Chart.js Bar Chart',
//       },
//     },
//     scales: {
//       y: {
//         beginAtZero: true,
//         ticks: {
//           // Include a dollar sign in the ticks
//           callback: function (value, index, values) {
//             return '$' + value
//           },
//           color: '#7c8ea7',
//         },
//         grid: {
//           //  drawBorder: 'border',
//           color: 'rgba(132, 145, 183, 0.15)',
//           //  borderDash: [3],
//           //  borderColor: 'rgba(132, 145, 183, 0.15)',
//         },
//         //  beginAtZero: true,
//       },
//       x: {
//         ticks: {
//           color: '#7c8ea7',
//         },
//         grid: {
//           display: false,
//           color: 'rgba(132, 145, 183, 0.09)',
//           //  borderDash: [3],
//           //  borderColor: 'rgba(132, 145, 183, 0.09)',
//         },
//       },
//     },
//   },
// }

// const polarChartConfig: ChartConfiguration = {
//   type: 'polarArea',
//   data: {
//     labels: ['Desktops', 'Laptop', 'Tablets', 'Mobiles'],
//     datasets: [
//       {
//         data: [80, 50, 100, 121],
//         backgroundColor: ['#4d79f6', '#ff5da0', '#e0e7fd', '#4ac7ec'],
//         borderColor: 'transparent',
//         hoverBackgroundColor: ['#4d79f6', '#ff5da0', '#e0e7fd', '#4ac7ec'],
//       },
//     ],
//   },
//   options: {
//     maintainAspectRatio: false,
//     plugins: {
//       legend: {
//         labels: {
//           color: '#7c8ea7',
//           font: {
//             family: 'Be Vietnam Pro',
//           },
//         },
//       },
//     },
//   },
// }

// const pieChartConfig: ChartConfiguration = {
//   type: 'pie',
//   data: {
//     labels: ['Desktops', 'Laptop', 'Tablets', 'Mobiles'],
//     datasets: [
//       {
//         data: [80, 50, 100, 121],
//         backgroundColor: ['#4d79f6', '#ff5da0', '#e0e7fd', '#4ac7ec'],
//         // cutout: 0,
//         radius: 100,
//         borderColor: 'transparent',
//         borderRadius: 0,
//         hoverBackgroundColor: ['#4d79f6', '#ff5da0', '#e0e7fd', '#4ac7ec'],
//       },
//     ],
//   },
//   options: {
//     maintainAspectRatio: false,
//     plugins: {
//       legend: {
//         labels: {
//           color: '#7c8ea7',
//           font: {
//             family: 'Be Vietnam Pro',
//           },
//         },
//       },
//     },
//   },
// }

// const radarChartConfig: ChartConfiguration = {
//   type: 'radar',
//   data: {
//     labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
//     datasets: [
//       {
//         label: 'Monthly Report',
//         data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
//         backgroundColor: ['rgba(11, 81, 183, 0.1)'],
//         borderColor: ['rgba(11, 81, 183, 1)'],
//         borderWidth: 2,
//         borderDash: [3],
//         borderJoinStyle: 'round',
//         borderCapStyle: 'round',
//         pointBorderColor: 'rgba(11, 81, 183, 1)',
//         pointRadius: 3,
//         pointBorderWidth: 1,
//         tension: 0.3,
//         fill: true,
//         hitRadius: 5,
//       },
//       {
//         label: 'Monthly Report',
//         data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
//         backgroundColor: ['rgba(28, 202, 184, 0.1)'],
//         borderColor: ['rgba(137, 153, 175, 0.3)'],
//         borderWidth: 2,
//         borderDash: [0],
//         borderJoinStyle: 'round',
//         borderCapStyle: 'round',
//         pointBorderColor: 'rgba(137, 153, 175, 0.3)',
//         pointRadius: 3,
//         pointBorderWidth: 1,
//         tension: 0.3,
//       },
//     ],
//   },
//   options: {
//     elements: {
//       line: {
//         borderWidth: 3,
//       },
//     },
//     maintainAspectRatio: false,
//     scales: {
//       r: {
//         angleLines: {
//           display: true,
//           color: 'rgba(137, 153, 175, 0.3)',
//           borderDash: [2],
//           // borderColor: [
//           //   'rgba(137, 153, 175, 0.3)',
//           // ],
//         },
//         grid: {
//           color: 'rgba(137, 153, 175, 0.3)',
//         },
//       },
//     },
//     plugins: {
//       legend: {
//         labels: {
//           color: '#7c8ea7',
//           font: {
//             family: 'Be Vietnam Pro',
//           },
//         },
//       },
//     },
//   },
// }

// export { donutChartConfig, barChartConfig, polarChartConfig, pieChartConfig, radarChartConfig }


export const lineData: ChartData<"line", any, any> = {
  labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
  datasets: [{
    label: 'Monthly Report',
    data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
    backgroundColor: [
      '#6f6af8',
    ],
    borderColor: [
      '#6f6af8',
    ],
    borderWidth: 2,
    borderDash: [3],
    borderJoinStyle: "round",
  },
  {
    label: 'Monthly Report',
    data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
    backgroundColor: [
      '#16cdc7',
    ],
    borderColor: [
      '#16cdc7',
    ],
    borderWidth: 2,
    borderDash: [0],
    borderJoinStyle: "round",
  }]
};

export const lineOpts: ChartOptions<'line'> = {
  maintainAspectRatio: false,
  plugins: {
    legend: {
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      },

    }
  },
  scales: {
    y: {
      beginAtZero: true,
      ticks: {
        // Include a dollar sign in the ticks
        callback: function (value, index, values) {
          return '$' + value;
        },
        color: '#7c8ea7',
      },
      grid: {
        color: 'rgba(132, 145, 183, 0.15)',
      },
    },
    x: {
      ticks: {
        color: '#7c8ea7',
      },
      grid: {
        display: false,
        color: 'rgba(132, 145, 183, 0.09)',
      }
    }
  },
};

export const doughnutData: ChartData<"doughnut", any, any> = {
  labels: [
    "Desktops",
    "Laptop",
    "Tablets",
    "Mobiles",],
  datasets: [{
    data: [80, 50, 100, 121],
    backgroundColor: [
      "#f67f7f",
      "#7777f0",
      "#16cdc7",
      "#6f6af8",
    ],
    borderColor: "transparent",
    borderRadius: 0,

    hoverBackgroundColor: [
      "#4d79f6",
      "#ff5da0",
      "#e0e7fd",
      "#4ac7ec",
    ],
  },]
};

export const doughnutOpts: ChartOptions<'doughnut'> = {
  cutout: 100,
  radius: 100,
  maintainAspectRatio: false,
  plugins: {
    legend: {
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      }
    }
  }
};

export const barData: ChartData<"bar", any, any> = {
  labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
  datasets: [{
    label: 'Monthly Report',
    data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
    borderRadius: 100,
    borderSkipped: false,
    backgroundColor: '#6f6af8',
    borderColor: '#6f6af8',
    borderWidth: 1,
    indexAxis: 'x',
    barThickness: 15,
    grouped: true,
    maxBarThickness: 9,
    barPercentage: 50
  },
  {
    label: 'Monthly Report',
    data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
    borderRadius: 100,
    borderSkipped: false,
    backgroundColor: '#16cdc7',
    borderColor: '#16cdc7',
    borderWidth: 1,
    indexAxis: 'x',
    barThickness: 15,
    grouped: true,
    maxBarThickness: 9,
  }]
};

export const barOpts: ChartOptions<'bar'> = {
  maintainAspectRatio: false,
  responsive: true,
  plugins: {
    legend: {
      position: 'top',
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      }
    },
    title: {
      display: false,
      text: 'Chart.js Bar Chart'
    }
  },
  scales: {
    y: {
      beginAtZero: true,
      ticks: {
        // Include a dollar sign in the ticks
        callback: function (value, index, values) {
          return '$' + value;
        },
        color: '#7c8ea7',
      },
      grid: {
        color: 'rgba(132, 145, 183, 0.15)',
      },
    },
    x: {
      ticks: {
        color: '#7c8ea7',
      },
      grid: {
        display: false,
        color: 'rgba(132, 145, 183, 0.09)',
      }
    }
  },
};

export const polarData: ChartData<"polarArea", any, any> = {
  labels: [
    "Desktops",
    "Laptop",
    "Tablets",
    "Mobiles",],
  datasets: [{
    data: [80, 50, 100, 121],
    backgroundColor: [
      "#4d79f6",
      "#ff5da0",
      "#e0e7fd",
      "#4ac7ec",
    ],
    borderColor: "transparent",
    hoverBackgroundColor: [
      "#4d79f6",
      "#ff5da0",
      "#e0e7fd",
      "#4ac7ec",
    ],
  },]
};

export const polarOpts: ChartOptions<'polarArea'> = {
  maintainAspectRatio: false,
  plugins: {
    legend: {
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      }
    }
  }
};

export const pieData: ChartData<"pie", any, any> = {
  labels: [
    "Desktops",
    "Laptop",
    "Tablets",
    "Mobiles",],
  datasets: [{
    data: [80, 50, 100, 121],
    backgroundColor: [
      "#4d79f6",
      "#ff5da0",
      "#e0e7fd",
      "#4ac7ec",
    ],
    borderColor: "transparent",
    borderRadius: 0,
    hoverBackgroundColor: [
      "#4d79f6",
      "#ff5da0",
      "#e0e7fd",
      "#4ac7ec",
    ],
  },]
};

export const pieOpts: ChartOptions<'pie'> = {
  radius: 100,
  maintainAspectRatio: false,
  cutout: 0,
  plugins: {
    legend: {
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      }
    }
  }
};

export const radarData: ChartData<"radar", any, any> = {
  labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
  datasets: [{
    label: 'Monthly Report',
    data: [12, 19, 13, 9, 12, 11, 12, 19, 13, 9, 12, 11],
    backgroundColor: [
      'rgba(11, 81, 183, 0.1)',
    ],
    borderColor: [
      'rgba(11, 81, 183, 1)',
    ],
    borderWidth: 2,
    borderDash: [3],
    borderJoinStyle: "round",
    borderCapStyle: "round",
    pointBorderColor: 'rgba(11, 81, 183, 1)',
    pointRadius: 3,
    pointBorderWidth: 1,
    tension: 0.3,
    fill: true,
    hitRadius: 5,
  },
  {
    label: 'Monthly Report',
    data: [8, 12, 15, 11, 8, 14, 16, 13, 10, 7, 19, 16],
    backgroundColor: [
      'rgba(28, 202, 184, 0.1)',
    ],
    borderColor: [
      'rgba(137, 153, 175, 0.3)',
    ],
    borderWidth: 2,
    borderDash: [0],
    borderJoinStyle: "round",
    borderCapStyle: "round",
    pointBorderColor: 'rgba(137, 153, 175, 0.3)',
    pointRadius: 3,
    pointBorderWidth: 1,
    tension: 0.3,
  }]
};

export const radarOpts: ChartOptions<'radar'> = {
  elements: {
    line: {
      borderWidth: 3,
    }
  },
  maintainAspectRatio: false,
  scales: {
    r: {
      angleLines: {
        display: true,
        color: 'rgba(137, 153, 175, 0.3)',
        borderDash: [2],

      },
      grid: {
        color: 'rgba(137, 153, 175, 0.3)',
      },
    },
  },
  plugins: {
    legend: {
      labels: {
        color: '#7c8ea7',
        font: {
          family: 'Be Vietnam Pro',
        }
      }
    }
  }
};


